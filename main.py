...
async def send_graph(message: types.Message):
    ...
    async def send_menu(message: types.Message):
        menu = "<menu>"
        await bot.send_message(message.from_user.id, f"Вот вывод вашего меню по результатам. Меню: {menu}")


    @dp.message_handler(lambda message: True)
    async def process_feedback(message: types.Message):
        cursor.execute(f"UPDATE test SET feedback = '{message.text}' WHERE user_id = {message.from_user.id}")
        await bot.send_message(message.from_user.id, "Спасибо за ваш отзыв! Он очень важен для нас.")
        conn.commit()
