...
async def send_graph(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, f"Вы согласны, что Ваша доминирующая эмоция – <b>{dom_emotion}</b>?",
                           reply_markup=keyboard, parse_mode=ParseMode.HTML)
        ...
        if callback_query.data == 'Yes':
            ...
            if dom_emotion == 'Печаль':
                ...
                await bot.send_message(callback_query.from_user.id, chill_menu, parse_mode=ParseMode.HTML,
                                       reply_markup=get_menu_keyboard())
            elif dom_emotion == "Печаль и Спокойствие":
                await bot.send_message(callback_query.from_user.id, pechal_and_chill_menu, parse_mode=ParseMode.HTML,
                                       reply_markup=get_menu_keyboard())
